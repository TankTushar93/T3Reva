...
const Modern = () => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 z-1 items-center justify-center w-full  gap-29 lg:gap-4 xl:gap-10 px-4 lg:px-2 xl:px-24 py-12">
        {plans.map((plan, index) => (
          <div key={index} className="relative flex items-center w-full lg:w-80 xl:w-full justify-center">
            <div className="h-auto w-full lg:w-92 relative px-10 py-10 z-10 bg-white border border-gray-300">
 ...
 className="text-lg text-gray-500">{item.title}</p>
            ...
            <div
              className={`absolute flex justify-center items-center z-0 w-full sm:w-full  md:w-full   lg:w-full  xl:w-85  lg:left-0 xl:left-18  ${
            ...
            >
              {plan.buttonLabel && (
                <a
                  ...
                  className={` ${plan.title === "Professional" ? 'mt-92 lg:mt-99' : 'mt-85 lg:mt-94'} items-center justify-canter  gap-3 group lg:ml-32 flex text-blue-500 text-right py-2 px-4 cursor-pointer `}
                >
