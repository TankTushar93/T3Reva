...
const Navbar = () => {
    ...
    return (
        ...
        <Topbar/>
            ...
            <nav style={{backgroundColor:background === 'Dark' ? col : headerMenu === 'Default' || headerMenu === 'Without Topbar' || headerMenu === 'Full Width' || headerMenu === 'Full Width Without Topbar' ? '#fff'  : col,
             ...
             className={`fixed top-0 left-0 w-full z-[9999] px-3 ${headerMenu === 'Full Width' || headerMenu === 'Full Width Without Topbar' || headerMenu === 'Full Width Transparent' ? 'px-0' : 'lg:px-21'} border-b border-white py-[29px] font-sans flex justify-between items-center hover:text-gray-800`}>
                ...
                <div className=' hidden lg:flex gap-9 mr-4 justify-center items-center'>
                    {navitems.map((item) => (
                        <div key={item.data.uid} style={{backgroundColor:background === 'Dark' ? col  : headerMenu === 'Default' || headerMenu === 'Without Topbar' || headerMenu === 'Full Width' || headerMenu === 'Full Width Without Topbar' ? '#fff'  : col,
                color:background === 'Dark' ? '#fff' : headerMenu === 'Default' || headerMenu === 'Without Topbar' || headerMenu === 'Full Width' || headerMenu === 'Full Width Without Topbar' ? 'black' :  '#fff'
             }}
              className={`relative flex group gap-2 justify-center items-center   text-gray-500 cursor-pointer `}>
                            <NavLink to={item.data.slug} className='cursor-pointer font-normal'>
                                {item.title}
                            </NavLink>
                            ...
                            {item.children && item.children.length > 0 && (
                                ...
                                className={`absolute top-full hidden mr-14  group-hover:flex  z-50`}>
                                 ...
                                 className={`absolute border  left-full top-0 mt-0  shadow-md  w-71 rounded-md flex flex-col gap-3 p-4`}>
                                                            {child.children.map((sub) => (
            ...
            {/*----------------- MObile View ------------------*/}
<div
  className={`fixed z-[9999] block w-full lg:hidden top-[89px] left-0 h-74 overflow-y-auto bg-white shadow-sm transform transition-transform duration-300 ${
                    ...
                    {navitems.map((item) => (
                        <li key={item.data.uid} className="flex flex-col">
                            ...
                            >
                                ...
                                {item.children?.length > 0 && <IoIosArrowDown className={`transition-transform ${openDropdown === item.title ? "rotate-180" : ""}`} />}
                            </div>
                            ...
                            {openDropdown === item.title && item.children?.length > 0 && (
                                <div className="ml-4 mt-3 flex flex-col gap-4">
                                    {item.children.map((child) => (
                                        <Link key={child.data.uid} to={child.data.slug} className="text-gray-500 hover:text-black">
                                            {child.title}
                                        </Link>
                                    ))}
                                </div>
